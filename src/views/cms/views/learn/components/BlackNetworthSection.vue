<template>
  <b-container class="hero-section">
    <h2 class="gordita-medium">Black Networth Section Content</h2>
    <form>
      <b-row class="mt-5">
        <b-col>
      ...
      <button @click.prevent="updateSection(false)" class="mt-5">
        <fai icon="upload" />
        Update Black Networth Section's Content
      </button>
      <button @click.prevent="updateSection(true)" class="mt-5 ml-1">
        <fai icon="magic" />
        Preview Changes to Black Networth Section
      </button>
    </form>
...
export default {
  ...
  methods: {
    updateSection: async function(update = false) {
      ...
      currentData.learnPage.blackNetworthSection = this.sectionData;
      if (update)
        this.previewContent(
          currentData,
        ...
        this.updateContent(
          currentData,
          "Black Networth Section Updated Successfully"
        );
    }
  },
  mounted() {
    Object.keys(this.sectionData).forEach(key => {
      this.sectionData[key] = this.websiteData.blackNetworthSection[key];
    });
  },
  props: {
    websiteData: Object
